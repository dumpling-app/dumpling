...
int       __wut_fs_open(struct _reent *r, void *fileStruct, const char *path, int flags, int mode);
...
int       __wut_fs_link(struct _reent *r, const char *existing, const char  *newLink);
int       __wut_fs_unlink(struct _reent *r, const char *name);
int       __wut_fs_chdir(struct _reent *r, const char *name);
int       __wut_fs_rename(struct _reent *r, const char *oldName, const char *newName);
...
int       __wut_fs_dirnext(struct _reent *r, DIR_ITER *dirState, char *filename, struct stat *filestat);
int       __wut_fs_dirclose(struct _reent *r, DIR_ITER *dirState);
int       __wut_fs_statvfs(struct _reent *r, const char *path, struct statvfs *buf);
